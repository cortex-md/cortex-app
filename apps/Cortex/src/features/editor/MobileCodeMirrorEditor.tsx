...
import { readonlyExtension } from "@cortex/editor/extensions"
...
function createCodeMirrorConfig(editorConfig: MobileCodeMirrorEditorProps["editorConfig"]) {
	return {
		...
		useSpaces: editorConfig.useSpaces,
...
export default function MobileCodeMirrorEditor({
	...
	useEffect(() => {
		flushPendingCommitRef.current = async () => {
			...
			await commitBodyRef.current(pendingCommit.body, pendingCommit.revision)
		}
