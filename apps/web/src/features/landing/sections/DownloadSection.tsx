...
const toneClassNames = {
	amber: {
		icon: "bg-accent-amber-subtle text-accent-amber-text",
		chip: "bg-accent-amber-subtle text-accent-amber-text",
	},
	sage: {
		icon: "bg-accent-sage-subtle text-accent-sage-text",
		chip: "bg-accent-sage-subtle text-accent-sage-text",
	},
	sky: {
		icon: "bg-accent-sky-subtle text-accent-sky-text",
		chip: "bg-accent-sky-subtle text-accent-sky-text",
	},
} as const
...
function AppleMark({ className = "" }: PlatformMarkProps) {
	return (
		<svg
			className={className}
			aria-hidden="true"
			data-platform-mark="macos"
			fill="currentColor"
			focusable="false"
			viewBox="0 0 24 24"
		>
...
function WindowsMark({ className = "" }: PlatformMarkProps) {
	return (
		<svg
			className={className}
			aria-hidden="true"
			data-platform-mark="windows"
			fill="currentColor"
			focusable="false"
			viewBox="0 0 24 24"
		>
			<path d="M0 3.449 9.75 2.1v9.451H0zm10.949-1.5L24 0v11.4H10.949zM0 12.6h9.75v9.451L0 20.699zm10.949 0H24V24l-13.051-1.351z" />
		</svg>
	)
}

function LinuxMark({ className = "" }: PlatformMarkProps) {
	return (
		<svg
			className={className}
			aria-hidden="true"
			data-platform-mark="linux"
			fill="currentColor"
			focusable="false"
			viewBox="0 0 24 24"
		>
...
function PlatformMark({ platform }: { platform: DownloadPlatform }) {
	if (platform.id === "macos") {
		return <AppleMark className="size-5" />
	}

	if (platform.id === "windows") {
		return <WindowsMark className="size-5" />
	}
	...
	return (
		...
		>
			<div className="flex items-start justify-between gap-4">
				<span className={`grid size-11 place-items-center rounded-lg ${tone.icon}`}>
			...
			<div className="mt-6 flex flex-wrap gap-2">
				{platform.chips.map((chip) => (
					<span
						className={`rounded-full px-3 py-1 text-[12px] leading-5 font-semibold ${tone.chip}`}
						key={`${platform.id}-${chip}`}
					>
						{chip}
					</span>
				))}
	...
	return (
		<section className="scroll-mt-24 py-24 max-md:py-[80px] max-sm:py-[68px]" id="downloads">
			<div className="mx-auto w-[min(1180px,calc(100%_-_64px))] max-lg:w-[min(calc(100%_-_48px),940px)] max-md:w-[min(calc(100%_-_36px),720px)] max-sm:w-[min(calc(100%_-_28px),520px)]">
				<div className="grid grid-cols-[minmax(0,0.72fr)_minmax(280px,0.28fr)] items-end gap-10 max-lg:grid-cols-1">
					<div>
						<h2 className="m-0 max-w-[760px] text-balance text-[clamp(34px,4.2vw,56px)] leading-[1.04] font-semibold tracking-[-0.018em]">
